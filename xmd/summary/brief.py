...
from ..digest.trending import DEFAULT_SIMILARITY, find_stories
...
def load_item_ids(export: Path, full: Path) -> list[str] | None:
    """The ids of every item a digest holds (the brief rebuilds its posts from them): the `.items.json` that
    `xmd digest` writes beside the export, or, for a digest made before that file existed, its full digest.
    None when neither is there. The full digest is optional now, so the ids file is the one to trust."""
    items_file = export.with_suffix(".items.json")

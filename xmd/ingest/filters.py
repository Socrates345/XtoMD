...
def _keep(item: FeedItem, config: Config) -> bool:
    ...
    if (
        ...
        return False
    # the adapters cut `title` at 200 characters: a keyword deeper in a long tweet, or in the post it quotes, is
    # only in the text. Each part is searched on its own, so a keyword can't match across the seam between two.
    parts = [p.lower() for p in (title, item.full_text or item.text, item.retweet_of_text) if p]
    if any(kw.lower() in part for kw in config.block_keywords for part in parts):
        return False
    return True
...
def _dedup_by_text(items: list[FeedItem]) -> list[FeedItem]:
    ...
    *different* people posting the same thing is never treated as a
    ...
    kept = []
    for item in items:
        content = _normalize(item.full_text or item.text or item.title)
        if not content:  # nothing to compare (an image-only post): each one is its own post
            kept.append(item)
            continue
        key = ((item.author or item.source).strip().lower(), content)
        if key in seen:
            continue
        seen.add(key)
        kept.append(item)

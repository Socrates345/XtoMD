...
from xmd.digest.render import build_markdown, build_quick
...
HOSTILE = (
    "look ![](https://tracker.example/p.gif) [click](https://phish.example/login) <img src=https://tracker.example/x.png> "
    "`$= dv.pages()` ![[a private note]] see https://t.co/abc123."
)


def _body_line(md: str, marker: str) -> str:
    """The line of a rendered digest that holds a tweet's text, found by a word of it (not the heading line)."""
    return next(ln for ln in md.splitlines() if marker in ln and not ln.startswith("####"))


def test_a_whole_tweet_cannot_inject_images_links_tags_embeds_or_code_but_keeps_its_links_clickable():
    """The one-liners escape tweet text already; the tweets shown whole (the full digest, and a priority source's
    in the quick digest) are written as markdown too, and X lets anyone write `![](...)` in a post."""
    for md in (
        build_markdown([_post(1, "@vip", HOSTILE)], NOW),
        build_quick([_post(1, "@vip", HOSTILE)], NOW, priority_sources=frozenset({"@vip"})),
    ):
        line = _body_line(md, "tracker.example/p.gif")
        for markup in ("![](", "![[", "[[", "<img"):
            assert markup not in line
        assert re.search(r"(?<!\\)[\[\]`]", line) is None  # no bracket or backtick left unescaped: no link, image, embed or code
        assert "!\\[\\](https://tracker.example/p.gif)" in line  # shown as text, its URL still a bare, clickable one
        assert "&lt;img src=https://tracker.example/x.png&gt;" in line
        assert line.endswith("see https://t.co/abc123.")  # a link stays bare and the sentence's full stop stays out of it


def test_a_link_cannot_smuggle_markdown_inside_its_own_characters():
    """`[` and `]` are legal in a URL, so an escaped body that left links alone would still let one carry an image."""
    text = "read https://a.example/![](https://tracker.example/p.gif)?x=[y](https://phish.example) now"
    line = _body_line(build_markdown([_post(1, "@vip", text)], NOW), "a.example")
    assert "![" not in line and "](" not in line
    assert "https://a.example/!%5B%5D(https://tracker.example/p.gif)" in line  # still one working link


@pytest.mark.parametrize("opener", [
    "# Breaking", "## X / finance", "1. first", "2) second", "- item", "+ item", "~~~", "$$", "%% hidden", "```js", "> quote", "*** ", "___",
])
def test_a_body_cannot_open_a_heading_a_list_a_fence_a_math_block_or_a_comment_at_the_start_of_its_line(opener):
    line = _body_line(build_markdown([_post(1, "@vip", f"{opener} and more")], NOW), "and more")
    assert not re.match(r"(#|\d+[.)]|[-+~$%>`*_])", line), line
    assert line.replace("\\", "").replace("&gt;", ">") == " ".join(f"{opener} and more".split())  # only escaped, nothing lost


def test_a_body_that_only_starts_like_markup_is_still_readable():
    """The escape must not garble ordinary tweets: a leading ticker, a percentage or a hashtag reads as typed."""
    for text, shown in (("$ACME up 5%", "$ACME up 5%"), ("-5% today", "-5% today"), ("#news of the day", "#news of the day")):
        line = _body_line(build_markdown([_post(1, "@vip", text)], NOW), text.split()[-1])
        assert line.replace("\\", "") == shown


def test_the_quoted_post_of_a_stranger_inside_a_priority_tweet_is_escaped_too():
    item = _post(1, "@vip", "worth a read", retweet_of_author="mal_lory", retweet_of_text=HOSTILE)
    md = build_quick([item], NOW, priority_sources=frozenset({"@vip"}))
    quoted = next(ln for ln in md.splitlines() if ln.startswith("🔁 Retweeted"))
    assert quoted.startswith("🔁 Retweeted @mal\\_lory: look !\\[\\](https://tracker.example/p.gif)")
    assert "<img" not in quoted and "![](" not in quoted and "[[" not in quoted

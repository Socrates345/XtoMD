...
def test_twitterapi_fetch(monkeypatch):
    ...
    from xmd.ingest.adapters import twitterapi as twitterapi_adapter
...
def test_twitterapi_page_guard_ends_a_fetch_whose_cursor_never_ends(monkeypatch, caplog):
    """A page of tweets the adapter skips (no url) that always says 'there is more' would page for ever, and
    twitterapi.io bills every tweet a page returns."""
    from xmd.ingest.adapters import MAX_PAGES
    from xmd.ingest.adapters import twitterapi as twitterapi_adapter

    page = {
        "status": "success", "has_next_page": True, "next_cursor": "same",
        "data": {"tweets": [{"text": "no url on this one", "createdAt": "Tue Dec 10 07:00:30 +0000 2024"}]},
    }
    calls = []
    ...
    def fake_get(*a, **kw):
        calls.append(1)
        if len(calls) > 200:
            raise AssertionError("the fetch never stopped")  # a missing guard fails here instead of hanging the suite
        return FakeJsonResponse(page)

    monkeypatch.setattr(twitterapi_adapter.httpx, "get", fake_get)

    with caplog.at_level(logging.WARNING, logger="xmd"):
        items = twitterapi_adapter.fetch(
            Source("@someone", "twitterapi", handle="someone"), api_key="k", max_items=20, max_age_hours=0
        )

    assert items == [] and len(calls) == MAX_PAGES
